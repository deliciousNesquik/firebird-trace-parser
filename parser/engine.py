...
class TraceLogParser:
    ...
    def __init__(
        self, rules: dict[str, regex.Pattern], handler: Optional[EventHandler] = None
    ):
        """
    ...
    def parse_file(
        self, filepath: Union[str, Path], encoding: str = "utf-8"
    ) -> List[EventBase]:
        """
    ...
    def _flush_block(self) -> Optional[EventBase]:
        ...
        event = self.handler.handle(
            self._current_block, self._pending_lines, self.rules
        )
